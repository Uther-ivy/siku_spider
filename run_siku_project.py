...
from params_setting import project_params, person_params
...
def run_project():
    ...
    lists = []
    start = project_params[0]  # person_params[0]开始数  person_params[1] 迭代次数 person_params[2]增加个数
    for a in range(project_params[1]):
        end = start + project_params[2]
        print(start, end)
