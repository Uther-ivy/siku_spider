...
#
#
# pooldb_test.py = pymysql.connect(
#     host='localhost',
#     port=3306,
#     user='root',
#     password='root',
...
def insert_cai(name, times):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = "insert into yunqi_cai(name, time) " \
          "values (%s,%s);"
    cur.execute(sql, (name, times))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def updata_cai(name, times):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = "update yunqi_cai set " \
          "time=%s " \
          "where " \
          "name=%s ;"
    cur.execute(sql, (times, name))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def inster_siku_id(cname,skid):
    pooldb = db.connection()
    cur = pooldb.cursor()
    sql = "insert into siku_id(cname,skid) " \
          "values (%s,%s);"
    cur.execute(sql, (cname,skid))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def insertaddoninfos(fid, typeid, mid, title, senddate, linkman, pnumber, ztzmoney, lxwh, sjxmnumber, zzjgdm, jsxz,
    ...
    print(fid, typeid, mid, title, senddate, linkman, pnumber, ztzmoney, lxwh, sjxmnumber, zzjgdm, jsxz,tarea, lxjb)
    cur.execute(sql, (fid, typeid, mid, title, senddate, linkman, pnumber, ztzmoney, lxwh, sjxmnumber, zzjgdm, jsxz,tarea, "''", lxjb))
    pooldb.commit()
    # cur.commit()
    cur.close()
    pooldb.close()
...
def searchperson(zyyzh,name):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = "select aid from yunqi_addon18 where " \
          f"(zyyzh='{zyyzh}' and name='{name}')"
    cur.execute(sql)
...
def insert_cydw(title, kid, qid, qymc, tyshxydm, name, idnum, role):
    ...
    cur.execute(sql, (title, kid, qid, qymc, tyshxydm, name, idnum, role))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def searchzmc(kid):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = f"select * from yunqi_addon17_zzlx where (kid='{kid}')"
    cur.execute(sql)
    data = cur.fetchall()
    cur.close()
    pooldb.close()
...
def insert_ht(kid, ba_number, ht_type, dj_number, money, fb_qymc, cb_qymc, cb_id, type, build, fb_xydm, cb_xydm,
    ...
    cur.execute(sql, (
    kid, ba_number, ht_type, dj_number, money, fb_qymc, cb_qymc, cb_id, type, build, fb_xydm, cb_xydm, lh_qymc,
    lh_xydm, register_time, signing_time, source))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def insert_dt(kid, title, number, cost, area, height, structure, grade, up_area, dw_area,
    ...
    cur.execute(sql, (kid, title, number, cost, area, height, structure, grade, up_area, dw_area,
                      layer, down_layer, long, wide, protect, scale, other, ztb_number, sg_number, xk_number,
                      zl_number, aq_number, shock, green, green_type, shocks, limit, set, rebar, steel))
    pooldb.commit()
    cur.close()
...
def search_sgxcaqy(idnum, tyshxydm):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = "select * from yunqi_addoninfos_sgxcaqy " \
          "where " \
          f"(idnum = '{idnum}'and tyshxydm ='{tyshxydm}')"
    cur.execute(sql)
    data = cur.fetchone()
    cur.close()
    pooldb.close()
...
def insert_sgxcaqy(title, kid, pid, qymc, tyshxydm, name, idnum, aqxkzbh, sgxkzbh, usertype):
    ...
    sql = "insert into yunqi_addoninfos_sgxcaqy(title,kid, pid, qymc, tyshxydm, name, idnum, aqxkzbh, sgxkzbh,usertype) " \
          "values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
    cur.execute(sql, (title, kid, pid, qymc, tyshxydm, name, idnum, aqxkzbh, sgxkzbh, usertype))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def search_sgxcglr(name, title):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = "select * from yunqi_addoninfos_sgxcglr " \
          "where " \
          f"(name = '{name}'and title='{title}');"
    cur.execute(sql)
    data = cur.fetchone()
    cur.close()
    pooldb.close()
...
def insert_sgxcglr(title, kid, pid, tyshxydm, name, idnum, ssqy, sgxkzbh, gwmc, zsbh, zsyxq, fzdw):
    ...
    cur.execute(sql, (title, kid, pid, tyshxydm, name, idnum, ssqy, sgxkzbh, gwmc, zsbh, zsyxq, fzdw))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def search_sgxctzzyr(name):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = "select * from yunqi_addoninfos_sgxctzzyr " \
          "where " \
          f"(name = '{name}');"
    cur.execute(sql)
    data = cur.fetchone()
...
def insert_sgxctzzyr(title, kid, pid, name, idnum, ssqy, tyshxydm, sgxkzbh, gz, zsbh, zsyxq, fzdw):
    ...
    sql = "insert into yunqi_addoninfos_sgxctzzyr(title, kid, pid, name, idnum, ssqy, tyshxydm, sgxkzbh, gz, zsbh, zsyxq, fzdw) " \
          "values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
    cur.execute(sql, (title, kid, pid, name, idnum, ssqy, tyshxydm, sgxkzbh, gz, zsbh, zsyxq, fzdw))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def search_sgxcjxsb(sbbabh):
    ...
    pooldb=db.connection()
    cur = pooldb.cursor()
    sql = "select * from yunqi_addoninfos_sgxcjxsb " \
          "where " \
          f"(sbbabh = '{sbbabh}');"
    cur.execute(sql)
    data = cur.fetchone()
...
def insert_sgxcjxsb(title, kid, pid, jxsbmc, cqdw, jxmodel, sgxkzbh, sydw, sybw, acdw, sbbabh, jhjcsj, jhccsj):
    ...
    cur.execute(sql, (title, kid, pid, jxsbmc, cqdw, jxmodel, sgxkzbh, sydw, sybw, acdw, sbbabh, jhjcsj, jhccsj))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def search_zyry(IDCARD,CENSORNUM,SPECIALTYTYP):
    pooldb = db.connection()
    cur = pooldb.cursor()
    sql = "select * from yunqi_addoninfos_zyry " \
          "where " \
          f"(IDCARD = '{IDCARD}'and CENSORNUM= '{CENSORNUM}'and SPECIALTYTYP='{SPECIALTYTYP}');"
    cur.execute(sql)
    data = cur.fetchone()
    cur.close()
    pooldb.close()
...
def insert_zyry(kid, CORPNAME, IDCARD, SPECIALTYTYP, TBWORKDUTYDIC, SPE, TECHTITLELEVEL, CORPID, PRJNUM, CORPCODE,
    ...
    cur.execute(sql, (kid, CORPNAME, IDCARD, SPECIALTYTYP, TBWORKDUTYDIC, SPE, TECHTITLELEVEL, CORPID, PRJNUM, CORPCODE,
                USERID, USERNAME, CENSORNUM, IDCARDTYPENUM))
    pooldb.commit()
    cur.close()
    pooldb.close()
...
def insert_base(data):
    ...
    if searchzmc(kid):
        ...
        # print(f'all {qymc} certs is  delete ')
        # delete_zzlx(kid)
    for cert in certs:
        print(cert)
